...
class KairosDBReader(object):
  ...
  def get_intervals(self):
    ...
    What I currently understand [that is, me, pcn] is that
    get_intervals returns a set (or rather, an IntervalSet) that
    contains Interval objects.  Each Interval is a start and end time.
    When a request is finally made, it should be done by combining any
    overlapping time ranges (intervals), and then issuing a request
    for each range of times and those will be the stats.  I don't get
    whether they're important here, though.
    ...
    It seems, then, that it's acceptable to wait a moment here while we
  ...
  def fetch(self, startTime, endTime):
    ...
    """
    return pyk_graphite.read_absolute(self.conn, self.metric_path, startTime, endTime)
